...
public:
    vector<vector<int>> groupThePeople(vector<int>& groupSizes) {
        unordered_map<int, vector<int>> tmpgrp;
        vector<vector<int>> sol;
        
        for(unsigned int i = 0; i < groupSizes.size(); ++i) {
            tmpgrp[groupSizes[i]].push_back(i);
            
            //~ check if the group is complete
            if (tmpgrp[groupSizes[i]].size() == groupSizes[i]) {
                sol.push_back(tmpgrp[groupSizes[i]]);
                tmpgrp[groupSizes[i]].clear();
            }
        }
